...
# Current max pos for numerics is 29
# Current max pos for time is 1
debug = True
...
# Eliminate spaces after 'p.'
# Put in spaces after real words
# replace positive; with positive.
...
for utt in range(0, len(data['AllDocuments'][0]['Document']['Utterances'])):
    ...
    for phrasePos in range(0, len(utterance['Phrases'])):
        ...
        if 'nuc ish' in phrase['PhraseText']:
            ...
            term = index[0]
            if term.lower() not in biomarkerList[phrasePos] and term.lower() + '/' not in phrase['PhraseText'] and '/' + term.lower() not in phrase['PhraseText'] and 'nuc ish' not in phrase['PhraseText']:
                isProbe = False
        ...
        if any(x in utterance['UttText'] for x in ['unlikely to yield a different result']):
            if 'Rejected By Lab - unlikely new result' not in conceptList[0]:
                ...
                print('added concept for reject')
        ...
        for mapPos in range(0, len(phrase['Mappings'])):
            ...
            for mcPos in range(0, len(mapping['MappingCandidates'])):
                ...
                for semPos in range(0, len(mapCan['SemTypes'])):
                    ...
                    if semType == 'genf' and 'rearrangement' in mapCan['CandidateMatched'].lower() or 'overexpression' in mapCan['CandidateMatched'].lower() or 'overexpress' in mapCan['CandidateMatched']:
                    ...
                    if semType in ['orch', 'phsu', 'topp']:
                        ...
                        if mapText.lower() not in biomarkerList[phrasePos] and ', nos' not in mapText.lower() and mapText.lower().replace('-', '') not in biomarkerList[phrasePos] \
                                and mapText.lower() not in ['expression', 'detected', 'fusions', 'fusion', 'presence', 'perform', 'favor', 'complete excision'] and not mapText.lower().startswith(
                            ...
                            if 'refused' in utterance['UttText'] or 'refuse' in utterance['UttText'] or 'declined' in utterance['UttText'] or 'decline' in utterance['UttText'] or 'not interested' in \
                                    utterance['UttText']:
                    ...
                    if semType in ['gngm']:
                            ...
                            if '-' in toAdd and toAdd not in ['pd-l1', 'pd-1'] and 'her-2' not in toAdd and not toAdd.endswith('-'):
                                ...
                                if 'of' in toAdd:
                                    ...
                                    if (toAdd not in ['pd-l1'] or toAdd not in itertools.chain.from_iterable(biomarkerList)) and 'her2' not in toAdd.lower().replace('-', '') and len(
                                            toAdd.split()) < 3 and not any(x in ' '.join(list(itertools.chain.from_iterable(biomarkerList))) for x in [toAdd.lower() + '-', '-' + toAdd.lower()])\
                        ...
                        # Finally, this is just for genes just named the gene name
                            ...
                            if mapCan['MatchedWords'][0] and not any(x in ' '.join(mapCan['MatchedWords']).lower() for x in ['locus', 'gene', 'clock']):
                                        ...
                                        if mapCan['MatchedWords'][0] not in biomarkerList[phrasePos]:
                                            ...
                                            if mapCan['MatchedWords'][0].lower() not in biomarkerList[phrasePos]:
                                                ...
                                                if len(mapCan['MatchedWords']) > 1:
                                                    if mapCan['MatchedWords'][1].lower() in ['receptor', 'receptors']:
                    ...
                    if semType == 'qlco':
                        ...
                        # Or strengths
                        if mapping['MappingCandidates'][mcPos]['CandidateMatched'].lower() in ['strong', 'weak', 'moderate', 'equivocal', 'patchy', 'atypical', 'indeterminate', 'patchy distribution', 'low']:
                    ...
                    if 'aapp' in mapCan['SemTypes'] and any(x in mapCan['SemTypes'] for x in ['rcpt', 'enzy', 'bacs']):
                        ...
                        if toAdd and toAdd not in biomarkerList[phrasePos] and toAdd not in ['to']:
                            ...
                            if toAdd and toAdd.lower().replace('-','') not in ' '.join(biomarkerList[phrasePos]).replace('-', '') and not (toAdd.lower() in
                                                ['sep', 'mismatch', 'proteins', 'ms', 'oncogene', 'receptors', 'cish', 'hormone receptor', 'fish']):
                                ...
                                if '-' in phrase['PhraseText']:
                                    if phrase['PhraseText'].replace('.', '').replace(',', '').split('-')[1].isnumeric():
                                        ...
                                        if toAdd not in ' '.join(biomarkerList[phrasePos]) and toAdd.lower() not in ['to', 'unsuccessful', 'expressed', 'expression', 'negative/absent', 'decreased', 'transplant'] and toAdd.lower() not in toAddOld \
    ...
    for i in range(0, len(finalBiomarkerResults)):
        ...
        if i > 0:
            if finalNumericResults[i-1] == [] and finalTimeResults[i] != [] and finalBiomarkerResults[i] == [] and finalConceptResults[i] == [] and finalQualifierResults[i] == [] and finalTimeResults == []:
